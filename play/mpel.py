...
class MultiprocessingEventLoop(object):
    ...
    def dummy_work(id, recv_input_pipe, send_return_pipe):
        import time
        import os
        time.sleep(5)
        return {"value": random.randint(5, 10), "pid": os.getpid()}
...
if __name__ == '__main__':
    mp = MultiprocessingEventLoop(
        device_ids=range(3)
    )
    mp.dummy_work = dummy_work
    res_list = []
    for rank in range(3):
        args = {}
        res = mp.call_async(rank=rank, action="dummy_work", **args)
        res_list.append(res)
    [print(i.gen()) for i in res_list]
